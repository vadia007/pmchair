...
var MooRainbow = new Class({
	options: {
		...
		startColor: [255, 0, 0],
		wheel: false,
		onComplete: (typeof($empty) != 'undefined' ? $empty : Class.empty),
		onChange: (typeof($empty) != 'undefined' ? $empty : Class.empty)
	},

	getPageOffset: function(e) {
		var pageOffset = {'x': 0, 'y': 0};
		if (e && typeof(e.page) != "undefined") {
			pageOffset['x'] = e.page.x;
			pageOffset['y'] = e.page.y;
		} else {
	...
	arrayContains: function(arr, item) {
		if (typeof(arr.test) != "undefined")
			return arr.test(item);
		else
	...
	initialize: function(el, options) {
		...
		this.$E = typeof($E) != "undefined" ? $E : function(selector) { return $(document.body).getElement(selector); };
	...
	OverlayEvents: function() {
		...
		inputs = this._copy(this.arrRGB).concat(this.arrHSB, this.hexInput);
		...
		inputs.each(function(el) {
			...
			el.addEvent('keydown', function(e) {
				self.eventKeydown.call(self, e, el)
			});
			el.addEvent('keyup', function(e) {
				self.eventKeyup.call(self, e, el)
			});
		}, this);
		...
		this.layout.drag = new (Drag.Base || Drag)(this.layout.cursor, {
			limit: lim,
		...
		this.layout.overlay2.addEvent('mousedown', function(e){
			//e = new Event(e);
			var pageOffset = this.getPageOffset(e);
			
			this.layout.cursor.setStyles({
				'top': pageOffset['y'] - this.layout.overlay.getTop() - curH,
				'left': pageOffset['x'] - this.layout.overlay.getLeft() - curW
			});
	...
	sliderEvents: function() {
		...
		this.layout.sliderDrag = new (Drag.Base || Drag)(this.layout.arrows, {
			limit: {y: lim},
			modifiers: {x: false},
		...
		this.layout.slider.addEvent('mousedown', function(e){
			...
			this.layout.arrows.setStyle(
				'top', pageOffset['y'] - this.layout.slider.getTop() + this.snippet('slider') - arwH
			);
	...
	wheelEvents: function() {
		var arrColors = this._copy(this.arrRGB);
		if (typeof(arrColors.extend) != "undefined")
			arrColors = arrColors.extend(this.arrHSB);
		else
			arrColors = arrColors.append(this.arrHSB);

		var self = this;
		arrColors.each(function(el) {
			el.addEvents({
				'mousewheel': function(e) {
					self.eventKeys.call(self, e, el);
				},
				'keydown': function(e) {
					self.eventKeys.call(self, e, el);
				}
		...
		[this.layout.arrows, this.layout.slider].each(function(el) {
			el.addEvents({
				'mousewheel': function(e) {
					self.eventKeys.call(self, e, self.arrHSB[0], 'slider');
				},
				'keydown': function(e) {
					self.eventKeys.call(self, e, self.arrHSB[0], 'slider');
				}
	...
	eventKeys: function(e, el, id) {
		...
		} else if (e.type == (Element.Events.mousewheel.base || Element.Events.mousewheel.type)) wheel = (e.wheel > 0) ? 1 : -1;

		if (this.arrayContains(this.arrRGB, el)) type = 'rgb';
		else if (this.arrayContains(this.arrHSB, el)) type = 'hsb';
		else type = 'hsb';
	...
	eventKeyup: function(e, el) {
		...
		if (el.className.test(/hexInput/)) {
			...
			if (!(n >= 48 && n <= 57) && (!this.arrayContains(['backspace', 'tab', 'delete', 'left', 'right'], k)) && el.value.length > 3) return;
		}
	...
	doLayout: function() {
		...
		this.layout.slider = this.$E('#' + idPrefix + 'slider');
		...
		var spanEl = new Element('span', {'styles': {'position': 'absolute'}, 'class': prefix + 'ballino'});
		if (typeof(spanEl.setHTML) != "undefined")
			spanEl.setHTML(" &deg;");
		else
			spanEl.set("html", " &deg;");
		
		if (typeof(spanEl.injectAfter) != "undefined")
			spanEl.injectAfter(HU);
		else
			spanEl.inject(HU, 'after');
		...
		this.layout.cursor = this.$E('#' + idPrefix + 'cursor');
		this.layout.arrows = this.$E('#' + idPrefix + 'arrows');
		this.chooseColor = this.$E('#' + idPrefix + 'chooseColor');
		this.layout.backup = this.$E('#' + idPrefix + 'currentColor');
		this.RedInput = this.$E('#' + idPrefix + 'rInput');
		this.GreenInput = this.$E('#' + idPrefix + 'gInput');
		this.BlueInput = this.$E('#' + idPrefix + 'bInput');
		this.HueInput = this.$E('#' + idPrefix + 'HueInput');
		this.SatuInput = this.$E('#' + idPrefix + 'SatuInput');
		this.BrighInput = this.$E('#' + idPrefix + 'BrighInput');
		this.hexInput = this.$E('#' + idPrefix + 'hexInput');

		this.arrRGB = [this.RedInput, this.GreenInput, this.BlueInput];
		this.arrHSB = [this.HueInput, this.SatuInput, this.BrighInput];
		this.okButton = this.$E('#' + idPrefix + 'okButton');
	...
	_copy: function(arr) {
		if (typeof(arr.copy) != "undefined")
			return arr.copy();
		else if (typeof(Array.clone) != "undefined")
